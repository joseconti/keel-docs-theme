Proyecto: **keel-docs-theme** — el tema canónico de documentación (guía de usuario final + portal de desarrollador) que mi sistema keel aplicará a la documentación de todos mis proyectos, de forma automática y durante años. No es la web de un producto: es un sistema de plantillas neutro de marca, versionado y reutilizable. Debes entregar una carpeta `design-handoff/` con la estructura exacta del brief (artefactos reales construidos + una SPEC que lo gobierna). Las dos reglas que más importan: construye una vez y reutiliza por manifiesto, y cuando algo no esté definido pregunta — no inventes. El brief completo, a continuación.
